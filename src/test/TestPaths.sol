...
contract TestPaths {
    ...
    function getPath0(uint256 amount) internal pure returns (CachedRouter.Path memory path) {
    ...
    function getPath3(uint256 amount) internal pure returns (CachedRouter.Path memory path) {
        // Optimal ETH -> LUSD path for amounts in <320, 900> ETH range
        path.amount = amount;
        path.subPathsV3 = new CachedRouter.SubPathV3[](2);
    ...
    function getPath4(uint256 amount) internal pure returns (CachedRouter.Path memory path) {
        ...
        path.subPathsV3[2] = CachedRouter.SubPathV3({percent: 20, path: abi.encodePacked(WETH, uint24(3000), LUSD)});
    }

    function getPath5(uint256 amount) internal pure returns (CachedRouter.Path memory path) {
