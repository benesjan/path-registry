...
contract TestPaths {
    ...
    function getPath0(uint256 amount) internal pure returns (IPathRegistry.Path memory path) {
        // Optimal ETH -> LUSD path for amounts under 0.3 ETH
        path.amount = amount;
        path.subPathsV2 = new IPathRegistry.SubPathV2[](1);

        address[] memory subPath1Addresses = new address[](2);
        subPath1Addresses[0] = WETH;
        subPath1Addresses[1] = LUSD;
        path.subPathsV2[0] = IPathRegistry.SubPathV2({percent: 100, path: subPath1Addresses});
    }
    ...
    function getPath3(uint256 amount) internal pure returns (IPathRegistry.Path memory path) {
        ...
        path.amount = amount;
        path.subPathsV3 = new IPathRegistry.SubPathV3[](2);
        ...
        path.subPathsV3[1] = IPathRegistry.SubPathV3({
            percent: 35,
    ...
    function getPath4(uint256 amount) internal pure returns (IPathRegistry.Path memory path) {
        ...
        path.amount = amount;
        path.subPathsV3 = new IPathRegistry.SubPathV3[](3);
        ...
        path.subPathsV3[2] = IPathRegistry.SubPathV3({percent: 20, path: abi.encodePacked(WETH, uint24(3000), LUSD)});
    }

    function getPath5(uint256 amount) internal pure returns (IPathRegistry.Path memory path) {
        ...
        subPath1Addresses[0] = WETH;
        subPath1Addresses[1] = LUSD;
        path.subPathsV2[0] = IPathRegistry.SubPathV2({percent: 45, path: subPath1Addresses});

        // 2.
        path.subPathsV3[0] = IPathRegistry.SubPathV3({percent: 45, path: abi.encodePacked(WETH, uint24(3000), LUSD)});
        ...
        // 4.
        path.subPathsV3[2] = IPathRegistry.SubPathV3({
            percent: 5,
            path: abi.encodePacked(WETH, uint24(10000), DAI, uint24(100), USDC, uint24(500), LUSD)
        });
    }

    function getBrokenPathUniV2(uint256 amount) internal pure returns (IPathRegistry.Path memory path) {
        path.amount = amount;
        path.subPathsV2 = new IPathRegistry.SubPathV2[](1);
